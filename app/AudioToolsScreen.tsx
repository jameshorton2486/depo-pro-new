...
export default function AudioToolsScreen({ onBack, initialFiles = [], onFilesChange }: { onBack: () => void; initialFiles?: File[]; onFilesChange?: (files: File[], replacedFile?: File, replacedSource?: File, processedAudit?: AudioProfile) => void }) {
  ...
  const selectedTools=tools.filter(tool=>profileIds.includes(tool.id)).sort((a,b)=>(a.chainOrder??999)-(b.chainOrder??999));
  ...
  async function processAudio() {
    if (!file) return;
    setBusy(true); setAudit(null); setDerivative(null); setProcessedFile(null); setProcessedSource(null); setMessage("Preserving the original locally…");
    try {
  ...
  return <main className="audio-tools-shell">
    ...
    <section className="audio-tools-layout">
      ...
      <section className="audio-tools-card">
        ...
        <input ref={fallbackInput} className="audio-tools-hidden-input" type="file" accept="audio/*,.wav,.flac" onChange={event => event.target.files?.[0] && selectFile(event.target.files[0])} />
