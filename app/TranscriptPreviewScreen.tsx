...
export default function TranscriptPreviewScreen({deposition,onBack,onNavigate}:{deposition:PreviewDeposition;onBack:()=>void;onNavigate:(destination:string)=>void}){
  ...
  return <main className="print-preview">
    ...
    <section className="print-preview-layout">
      ...
      <div className="print-preview-content"><FinalizationPanel depositionId={deposition.id} onNavigate={onNavigate}/><div aria-live="polite">{loading?<div className="transcript-empty">Paginating the current rendered transcript…</div>:null}{error?<div className="transcript-empty"><h3>Preview unavailable</h3><p>{error}</p><button type="button" className="primary-button" onClick={onBack}>Return to Workspace</button></div>:null}
