...
export function localApiPort(environment=process.env){
  const raw=environment.LOCAL_API_PORT,port=raw===undefined||raw===""?4317:Number(raw);
  if(!Number.isInteger(port)||port<1||port>65535)throw new Error(`LOCAL_API_PORT is "${raw}", which is not a port number.`);
  return port;
}
