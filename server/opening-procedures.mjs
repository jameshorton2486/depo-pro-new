...
const SCRIPT_DEFINITIONS = Object.freeze({
  opening: {
    ...
    available:true, reviewStatus:"reviewed", expectedSource:null,
  ...
  interpreterOath: {
    ...
    available:false, reviewStatus:"source-required",
    expectedSource:"An interpreter oath in the approved wording for the jurisdiction this deposition is taken in.",
  ...
  witnessOath: {
    ...
    classification:"UNVERIFIED",
    available:false, reviewStatus:"source-required",
    expectedSource:"A witness oath and an affirmation in the approved wording for the jurisdiction this deposition is taken in.",
...
export function getOpeningProjection(root,{depositionId,storageRoot,scriptDefinitions=SCRIPT_DEFINITIONS}={}){
  ...
  const scripts=Object.entries(scriptDefinitions).map(([id,definition])=>({id,...definition,...renderScript(definition,tokens),...state.scripts[id],applicable:id!=="interpreterOath"||state.interpreterDisposition!=="NOT_APPLICABLE"}));
  ...
  const scriptReady=id=>{const item=scripts.find(entry=>entry.id===id);return Boolean(item?.available)&&item.reviewStatus==="reviewed"&&item.missing.length===0};
  const readiness={
