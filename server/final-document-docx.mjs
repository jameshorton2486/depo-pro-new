...
export function createFixedPageDocxSpec(printModel){
  ...
  const pages=(printModel.pages??[]).map(page=>({id:page.id,pageNumber:page.pageNumber,lines:page.lines.map(line=>({position:line.position,text:line.content,occupied:line.occupied,paragraphId:line.paragraphId,fragmentIds:(line.fragments??[]).map(fragment=>fragment.id),sourceWordIds:(line.fragments??[]).map(fragment=>fragment.sourceWordId).filter(Boolean)}))}));
  ...
  const unsigned={schemaVersion:"1.0.0",recordType:"FIXED_PAGE_DOCX_RENDERING_SPEC",renderer:"DEPO_PRO_INTERNAL_FIXED_PAGE_OOXML_V1",modelHash:printModel.modelHash,source:printModel.source,profile:printModel.layoutProfile,pages};
...
export function createTranscriptDocxArtifact(root,{depositionId,printModel,storageRoot,outputDirectory=null}={}){
  ...
  const spec=createFixedPageDocxSpec(printModel),specPath=path.join(directory,"professional-testimony-rendering-spec.json"),outputPath=path.join(directory,"professional-testimony.docx"),mappingPath=path.join(directory,"professional-testimony-line-map.json");
