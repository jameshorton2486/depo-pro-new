...
test("RX processing preserves the immutable original", async t => {
  ...
  const result=await createRxDerivative(value.root,value.audit,{originalPath:value.originalPath,recordAuditEvent:async()=>{},pythonExecutable:value.dependency,workerPath:value.dependency,pluginPath:value.dependency,inspectRxStatus:()=>({available:true,executable:value.dependency}),
    ...
    runEncoder:async(_command,args)=>fs.writeFileSync(args.at(-1),"lossless derivative"),measureQuality:async()=>({meanVolumeDb:-23,peakDbfs:-2,dynamicRangeDb:20,clippedSampleCount:0,lowFrequencyMeanDb:-45}),
    validateAudio:async()=>({durationSeconds:1,sampleRate:48000,channels:1,sampleFrames:48000})});
  assert.deepEqual(fs.readFileSync(value.originalPath),value.original); assert.equal(result.sourceSha256,value.audit.storage.original.sha256); assert.equal(result.sourceImmutable,true); assert.match(result.key,/\.flac$/); assert.deepEqual(result.outputEncoding,{container:"flac",sampleFormat:"s16",bitDepth:16,lossless:true});
});
