...
import { OPENING_STEPS, getOpeningProjection, saveOpeningState } from "../server/opening-procedures.mjs";
...
function scratch() {
  ...
  fs.writeFileSync(path.join(folder, "intake", "canonical-deposition-record.json"), JSON.stringify({
    case: { caseStyle: { value: "A v. B", source: "NOD_EXTRACTED", state: "EXTRACTED" } },
    deposition: { witness: { value: "W", source: "NOD_EXTRACTED", state: "EXTRACTED" } },
    counsel: [{ id: "c1", fullName: { value: "Ann Counsel", source: "NOD_EXTRACTED", state: "EXTRACTED" } }],
  }));
  return { depositionId, storageRoot, cleanup: () => fs.rmSync(storageRoot, { recursive: true, force: true }) };
}

const project = (s, state) => {
  if (state) saveOpeningState(null, { depositionId: s.depositionId, storageRoot: s.storageRoot, state });
  return getOpeningProjection(null, { depositionId: s.depositionId, storageRoot: s.storageRoot });
};
const script = (projection, id) => projection.scripts.find((item) => item.id === id);

test("the oaths are source-required stubs, and say where the text must come from", () => {
  const s = scratch();
  const projection = project(s);
  for (const id of ["witnessOath", "interpreterOath"]) {
    const item = script(projection, id);
    assert.equal(item.available, false, `${id} has no approved wording`);
    assert.equal(item.reviewStatus, "source-required");
    assert.match(item.expectedSource, /approved wording for the jurisdiction/,
      "a stub that does not say what it needs is not loud, it is just broken");
  }
  s.cleanup();
});

test("selecting an oath does not make the step ready while the text is a stub", () => {
  ...
  for (const selection of ["OATH", "AFFIRMATION"]) {
    ...
    assert.equal(projection.state.witnessOathSelection, selection, "the choice is still recorded");
    assert.equal(projection.readiness.witnessOath, false, `${selection} must not make an unapproved oath ready`);
  }
  s.cleanup();
});
...
test("an interpreter oath that is required cannot be ready either", () => {
  const s = scratch();
  assert.equal(project(s, { interpreterDisposition: "REQUIRED" }).readiness.interpreterOath, false);
  ...
  assert.equal(project(s, { interpreterDisposition: "NOT_APPLICABLE" }).readiness.interpreterOath, true);
  s.cleanup();
});
...
test("the whole screen cannot reach complete while an oath is a stub", () => {
  // What the ruling is actually protecting: the number the reporter reads mid-proceeding.
  const s = scratch();
  const projection = project(s, {
    witnessOathSelection: "OATH", interpreterDisposition: "NOT_APPLICABLE", examiningAttorneyId: "c1",
    verifiedFields: Object.fromEntries(["case.caseStyle", "case.causeNumber", "case.court", "deposition.witness",
      "deposition.depositionDate", "deposition.actualStart", "deposition.location", "reporter.fullName"].map((k) => [k, true])),
...
test("readiness is still reported, never enforced", () => {
  ...
  const s = scratch();
  const projection = project(s);
  assert.ok(Object.values(projection.readiness).some((value) => value === false));
  assert.ok(!("blocked" in projection) && !("canRecord" in projection),
    "the projection reports readiness and does not decide whether anything may happen");
  s.cleanup();
});
...
test("an unreviewed script with nothing left to fill is still not ready", () => {
  ...
  saveOpeningState(null, { depositionId: s.depositionId, storageRoot: s.storageRoot, state: { witnessOathSelection: "OATH" } });
  const definitions = {
    witnessOath: {
      title: "Witness Oath / Affirmation", classification: "UNVERIFIED",
      available: false, reviewStatus: "source-required", expectedSource: "Approved wording.",
      whenToUse: "Before testimony.",
      template: "Do you swear the testimony you are about to give is the truth?",
    },
  };
  const projection = getOpeningProjection(null, { depositionId: s.depositionId, storageRoot: s.storageRoot, scriptDefinitions: definitions });
  const item = projection.scripts.find((entry) => entry.id === "witnessOath");
  assert.equal(item.missing.length, 0, "the fixture must have nothing left to fill, or it proves nothing");
  assert.equal(projection.readiness.witnessOath, false, "unreviewed wording cannot be ready even when it is complete");

  // And the same script, once reviewed, is ready -- so the refusal tracks approval and not something else.
  const reviewed = { witnessOath: { ...definitions.witnessOath, available: true, reviewStatus: "reviewed", expectedSource: null } };
  const after = getOpeningProjection(null, { depositionId: s.depositionId, storageRoot: s.storageRoot, scriptDefinitions: reviewed });
...
test("a reviewed script with an unfilled token is not ready either", () => {
  ...
  const withHole = {
    witnessOath: {
      title: "Witness Oath", classification: "APPROVED_REPORTER_TEMPLATE",
      available: true, reviewStatus: "reviewed", expectedSource: null, whenToUse: "Before testimony.",
      template: "We are on the record at [ACTUAL TIME]. Do you swear to tell the truth?",
    },
  };
  const projection = getOpeningProjection(null, { depositionId: s.depositionId, storageRoot: s.storageRoot, scriptDefinitions: withHole });
  const item = projection.scripts.find((entry) => entry.id === "witnessOath");
  assert.deepEqual(item.missing, ["ACTUAL TIME"], "the fixture must actually have a hole, or it proves nothing");
  assert.equal(item.reviewStatus, "reviewed", "and it must be approved, so approval is not what refuses it");
  assert.equal(projection.readiness.witnessOath, false);
  s.cleanup();
});
