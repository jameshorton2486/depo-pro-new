...
test("429 and all server failures retry",async()=>{for(const status of [429,500,502,503,504,529,599]){let calls=0;const result=await fetchExternal("https://example.invalid",{}, {...options,fetchImpl:async()=>response(++calls===1?status:200)});assert.equal(result.status,200);assert.equal(calls,2)}});
