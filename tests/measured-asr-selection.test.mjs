...
test("recording a comparison does not change the selected ASR source",async t=>{
  ...
  const reference="Jane Doe did not attend the examination yesterday",terms=["Jane Doe","did not"],groups={properNames:["Jane Doe"]};
  await recordComparison(root,audit,{source:"original",...compareTranscripts(reference,"Jane Doe did attend examination yesterday",terms,groups)});
  await recordComparison(root,audit,{source:"processed",derivativeOperationId:operationId,...compareTranscripts(reference,reference,terms,groups)});
...
test("explicit selection retains original when the RX candidate loses a negation",async t=>{
  ...
  await recordComparison(root,audit,{source:"original",...compareTranscripts(reference,"No I did not authorize payment August fifty twenty six")});
  await recordComparison(root,audit,{source:"processed",derivativeOperationId:operationId,...compareTranscripts(reference,"I did authorize the payment on August 15 2026")});
