...
for(const profile of Object.values(RX_PROFILES)) test(`installed ${profile.module} profile renders sample-aligned disposable audio`,{skip:!enabled,timeout:120000},async t=>{
  ...
  assert.equal(result.profileId,profile.id); assert.equal(result.module,profile.module); assert.equal(result.sampleAligned,true); assert.equal(result.sourceSha256,sha256); assert.ok(fs.existsSync(path.join(root,"data",...result.key.split("/"))));
});
